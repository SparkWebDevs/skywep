define(["require", "exports", './DataEngine', './CurrencyManager'], function (require, exports, __ge, __cm) {
    ...
    var LightGridController = (function () {
        ...
        LightGridController.prototype.setData = function (data, colDefinitions) {
            ...
            if (!this.dataEngine) {
                ...
                var coldef = this.colDefinitions.findItem(function (cdl) {
        ...
        LightGridController.prototype.resized = function () {
            ...
            if (newwidth == -1)
                return; // there is no flex column (or it was fixed by user)
        ...
        LightGridController.prototype.processDataRows = function () {
            ...
            setTimeout(function () {
                ...
                setTimeout(function () {
                    self.resized();
                }, 50); // re-process the resized - after a short break the scrollbars have settled and we can do a proper assessment...  hmmmm... :(
            }, 0);
        };
        // async - clears and re-creates the datarows part of the grid - called when paging!
        LightGridController.prototype.refreshPageChange = function (action) {
            var self = this;
            // show the loading spinner
            self.$sgloading.css('display', 'flex');
            // disconnect the work from the call to allow the grid to show the loading spinner
        ...
        LightGridController.prototype.attachDelegatedHandlers = function () {
            ...
            $(".btnPrevPage", self.$pager).click(function () {
                self.refreshPageChange("prev");
            });
            $(".btnNextPage", self.$pager).click(function () {
                self.refreshPageChange("next");
            });
            $(".btnLastPage", self.$pager).click(function () {
                self.refreshPageChange("last");
            });
            // 
            ...
            $grid.on("mouseenter", ".cell-header", function () {
                if (self.isResizingColumn || self.isDraggingColumn)
                    return;
                cellHeader = $(this);
